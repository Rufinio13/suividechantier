...
export function TacheFormModal({
      ...
      for (const day of days) {
        ...
        if (conflict && conflict.chantierids && conflict.chantierids.length > 1) {
          ...
          if (otherIds.length > 0) {
            const otherNames = otherIds.map(id => chantiers.find(c => c.id === id)?.nomchantier).filter(Boolean);
            return { message: `Artisan en conflit le ${format(day, 'dd/MM/yyyy')} avec: ${otherNames.join(', ')}` };
          }
        }
      }
    } catch (err) { console.error("Erreur parsing conflit:", err); }
    return null;
  ...
  useEffect(() => {
    ...
    if (tache) {
      ...
      setFormData({
        nom: tache.nom || "", description: tache.description || "",
  ...
  const assignableEntities = useMemo(() => {
    ...
    const stOpts = sousTraitants?.filter(st => Array.isArray(st.assigned_lots) && st.assigned_lots.includes(lotName))
      .map(st => ({ id: st.id, nom: st.nomsocieteST || `${st.PrenomST || ""} ${st.nomST || ""}`.trim() || "Artisan", assignetype: "soustraitant" })) || [];
    const fOpts = fournisseurs?.filter(f => Array.isArray(f.assignedlots) && f.assignedlots.includes(lotName))
      .map(f => ({ id: f.id, nom: f.nomsocieteF || f.nomcontact || "Fournisseur", assignetype: "fournisseur" })) || [];
  ...
  useEffect(() => {
    if (formData.assigneid) {
      const stillValid = assignableEntities.some(e => e.id === formData.assigneid && e.assignetype === formData.assignetype);
      if (!stillValid) setFormData(prev => ({ ...prev, assigneid: "", assignetype: "" }));
    }
  }, [assignableEntities]);

  const handleSelectChange = (name, value) => {
    if (name === "assigneCombined") {
      if (value) { const [assignetype, assigneid] = value.split(":"); setFormData(prev => ({ ...prev, assignetype, assigneid })); }
      else setFormData(prev => ({ ...prev, assignetype: "", assigneid: "" }));
    }
    if (name === "lotid") setFormData(prev => ({ ...prev, lotid: value, assigneid: "", assignetype: "" }));
  };
  ...
  const handleSubmit = async e => {
    ...
    if (!formData.datedebut || !formData.duree) { alert("Veuillez renseigner la date de début et la durée."); return; }
    const deb = parseISO(formData.datedebut);
    if (!isValid(deb)) { alert("Date de début invalide"); return; }

    const datefin = calculateDateFinLogic(formData.datedebut, parseInt(formData.duree, 10));
    const payload = {
      nom: formData.nom, description: formData.description || null,
      chantierid: chantierId, lotid: formData.lotid,
      datedebut: formData.datedebut, datefin,
      assigneid: formData.assigneid || null, assignetype: formData.assignetype || null,
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer la tâche "${tache.nom}" ?`)) {
      try { await deleteTache(tache.id); onClose(); }
      catch (err) { console.error("❌ Erreur suppression tâche:", err); alert(`Erreur: ${err.message}`); }
    }
  };
      ...
      <DialogContent className="sm:max-w-[520px] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <select id="lotid" name="lotid" value={formData.lotid} onChange={(e) => handleSelectChange("lotid", e.target.value)} required
              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
              <option value="">Choisir un lot...</option>
              {sortedLots.map(l => <option key={l.id} value={l.id}>{l.lot}</option>)}
            </select>
          </div>
          <div className="space-y-2">
            <Label htmlFor="assigneCombined">Assigné à</Label>
            <select id="assigneCombined" name="assigneCombined"
              ...
              {assignableEntities.map(e => <option key={`${e.assignetype}-${e.id}`} value={`${e.assignetype}:${e.id}`}>{e.nom}</option>)}
            </select>
          </div>
          {tache && (
            <div>
              {(tache.artisan_termine || tache.constructeur_valide) && (
                <div className="space-y-2 mb-3 p-3 rounded-md border bg-gray-50 border-gray-300">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-700">L'artisan a marqué cette tâche comme terminée</span>
                  </div>
                  {tache.artisan_termine_date && <p className="text-xs text-gray-600">Date : {format(new Date(tache.artisan_termine_date), 'dd/MM/yyyy à HH:mm')}</p>}
                  {tache.artisan_photos && tache.artisan_photos.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold mb-2 text-gray-700">📸 {tache.artisan_photos.length} photo(s)</p>
                      <div className="grid grid-cols-4 gap-2">
                        {tache.artisan_photos.map((photo, idx) => (
                          <img key={idx} src={photo.url} alt={photo.name || `Photo ${idx + 1}`}
                            className="w-full h-20 object-cover rounded border cursor-pointer hover:opacity-80"
                            onClick={() => window.open(photo.url, '_blank')} title="Cliquer pour agrandir" />
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {(tache.artisan_termine || tache.constructeur_valide) && (
                <div className={`flex items-center space-x-2 p-3 rounded border mb-3 ${formData.constructeur_valide ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
                  <input type="checkbox" id="valider_tache" checked={formData.constructeur_valide || false}
                    onChange={(e) => setFormData(prev => ({ ...prev, constructeur_valide: e.target.checked, terminee: e.target.checked }))}
                    className="h-4 w-4 rounded border-gray-300 cursor-pointer" />
                  <Label htmlFor="valider_tache" className={`cursor-pointer font-medium ${formData.constructeur_valide ? 'text-green-900' : 'text-yellow-800'}`}>
              ...
              {!tache.artisan_termine && !tache.constructeur_valide && (
                <div className="flex items-center space-x-2 p-3 bg-slate-50 rounded border">
                  <input type="checkbox" id="terminee" checked={formData.terminee || false}
                    onChange={(e) => setFormData(prev => ({ ...prev, terminee: e.target.checked }))}
                    className="h-4 w-4 rounded border-gray-300 cursor-pointer" />
                  <Label htmlFor="terminee" className="cursor-pointer font-medium">Marquer comme terminée</Label>
                </div>
              )}
