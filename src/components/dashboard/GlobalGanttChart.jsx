...
import {
  parseISO, format, startOfDay, endOfDay, startOfWeek, endOfWeek,
  addWeeks, subWeeks, isPast, eachDayOfInterval, eachWeekOfInterval,
  isSameDay, isSameWeek, isWeekend, nextMonday, getMonth, getDate
} from 'date-fns';
...
const JOURS_FERIES = [
  { month: 1, day: 1 }, { month: 5, day: 1 }, { month: 5, day: 8 },
  { month: 7, day: 14 }, { month: 8, day: 15 }, { month: 11, day: 1 },
  { month: 11, day: 11 }, { month: 12, day: 25 },
];

const isJourFerie = (date) => { const m = getMonth(date) + 1; const d = getDate(date); return JOURS_FERIES.some(jf => jf.month === m && jf.day === d); };
const isValidDate = (date) => date instanceof Date && !isNaN(date.getTime());
const safeParseDate = (value) => { if (!value || typeof value !== 'string') return null; const p = parseISO(value); return isValidDate(p) ? p : null; };
const isCurrentWeek = (weekStart) => isSameWeek(weekStart, new Date(), { weekStartsOn: 1 });
...
export function GlobalGanttChart({ chantiers, taches, initialStartDate, sousTraitants }) {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 640) setChantierColWidth(CHANTIER_COL_WIDTH_MOBILE);
      else if (window.innerWidth < 1024) setChantierColWidth(CHANTIER_COL_WIDTH_TABLET);
      else setChantierColWidth(CHANTIER_COL_WIDTH_DESKTOP);
    };
    handleResize();
    ...
    tachesNonValidees.forEach(tache => {
      if (tache.assignetype === 'soustraitant' && tache.assigneid && tache.datedebut && tache.datefin) {
        ...
        days.forEach(day => {
          ...
          stAssignments[dayKey][stId].push({ tacheId: tache.id, chantierid: tache.chantierid });
        });
      }
    });
    Object.keys(stAssignments).forEach(dayKey => {
      Object.keys(stAssignments[dayKey]).forEach(stId => {
        const chantierIds = [...new Set(stAssignments[dayKey][stId].map(a => a.chantierid))];
  ...
  const getTaskColor = (tache, hasConflict) => {
    if (hasConflict) return 'bg-red-600';
    if (tache.artisan_termine && !tache.constructeur_valide) return 'bg-yellow-500';
    if (tache.constructeur_valide || tache.terminee) return 'bg-blue-500';
    const dateFinTache = safeParseDate(tache.datefin);
    if (dateFinTache && isPast(endOfDay(dateFinTache))) return 'bg-orange-500';
    return 'bg-green-500';
  };
  ...
  const ganttData = useMemo(() => {
    ...
    if (chantiers.length === 0) {
      ...
      return { items: [], overallStartDate: defaultStartWeek, overallEndDate: endOfWeek(addWeeks(defaultStartWeek, 4), { weekStartsOn: 1 }) };
    }
    const items = chantiers.map((chantier) => {
      ...
      chantierTaches.forEach(tache => { const s = safeParseDate(tache.datedebut); if (s && (!earliestTaskDate || s < earliestTaskDate)) earliestTaskDate = s; });
      const daysTasks = new Map();
      chantierTaches.forEach(tache => {
        const s = safeParseDate(tache.datedebut); const e = safeParseDate(tache.datefin);
        if (s && e) {
          const days = eachDayOfInterval({ start: startOfDay(s), end: endOfDay(e) });
          days.forEach(day => {
            if (!isWeekend(day)) {
              const key = format(day, 'yyyy-MM-dd');
              if (!daysTasks.has(key)) daysTasks.set(key, []);
              daysTasks.get(key).push(tache);
            }
          });
        }
      });
      return { id: chantier.id, name: chantier.nomchantier, daysTasks, tasks: chantierTaches, earliestTaskDate: earliestTaskDate || new Date(8640000000000000) };
    });
    items.sort((a, b) => a.earliestTaskDate - b.earliestTaskDate);
    let overallStartDate = null, overallEndDate = null;
    items.forEach(item => {
      item.tasks.forEach(tache => {
        const s = safeParseDate(tache.datedebut); const e = safeParseDate(tache.datefin);
        if (s && (!overallStartDate || s < overallStartDate)) overallStartDate = s;
        if (e && (!overallEndDate || e > overallEndDate)) overallEndDate = e;
      });
    });
    const oneWeekBefore = startOfWeek(subWeeks(todayWorkday, 1), { weekStartsOn: 1 });
    if (!overallStartDate) overallStartDate = oneWeekBefore;
    else if (overallStartDate > oneWeekBefore) overallStartDate = oneWeekBefore;
    if (!overallEndDate || todayWorkday > overallEndDate) overallEndDate = addWeeks(todayWorkday, 8);
  ...
  if (items.length === 0 && chantiers.length > 0) return <p className="text-muted-foreground text-center py-8">Aucune tâche planifiée pour les chantiers en cours.</p>;
  if (chantiers.length === 0) return <p className="text-muted-foreground text-center py-8">Aucun chantier en cours à afficher.</p>;
  ...
  const weekHeaders = useMemo(() => {
    const weeks = eachWeekOfInterval({ start: overallStartDate, end: overallEndDate }, { weekStartsOn: 1 });
    return weeks.map(weekStart => {
      const weekEnd = endOfWeek(weekStart, { weekStartsOn: 1 });
      const daysInWeek = eachDayOfInterval({ start: weekStart > overallStartDate ? weekStart : overallStartDate, end: weekEnd < overallEndDate ? weekEnd : overallEndDate });
      return { startDate: weekStart, label: format(weekStart, "'S'ww", { locale: fr }), isCurrentWeek: isCurrentWeek(weekStart), daysCount: daysInWeek.length };
    });
  }, [overallStartDate, overallEndDate]);

  const monthHeaders = useMemo(() => {
    const months = [];
    let currentMonth = null, dayCount = 0;
    allDays.forEach((day, index) => {
      const monthKey = format(day, 'yyyy-MM');
      if (monthKey !== currentMonth) {
        if (currentMonth !== null) months.push({ monthKey: currentMonth, name: format(allDays[index - 1], 'MMM yyyy', { locale: fr }), daysCount: dayCount });
        currentMonth = monthKey; dayCount = 1;
      } else { dayCount++; }
      if (index === allDays.length - 1) months.push({ monthKey: currentMonth, name: format(day, 'MMM yyyy', { locale: fr }), daysCount: dayCount });
    });
    return months.map(m => ({ ...m, width: m.daysCount * (dayWidth + DAY_GAP) }));
  }, [allDays, dayWidth]);

  const getDayPosition = (dayIndex) => dayIndex * (dayWidth + DAY_GAP);
  ...
  useEffect(() => {
    if (scrollContainerRef.current && allDays.length > 0 && !hasScrolledToToday.current) {
      const scrollToToday = () => {
        const mondayIndex = allDays.findIndex(day => isSameDay(day, startOfWeek(new Date(), { weekStartsOn: 1 })));
        if (mondayIndex !== -1) { scrollContainerRef.current.scrollLeft = Math.max(0, getDayPosition(mondayIndex)); hasScrolledToToday.current = true; }
        else {
          const todayIndex = allDays.findIndex(day => isSameDay(day, targetScrollDay));
          if (todayIndex !== -1) { scrollContainerRef.current.scrollLeft = Math.max(0, getDayPosition(todayIndex) - 100); hasScrolledToToday.current = true; }
        }
      };
      setTimeout(scrollToToday, 100); setTimeout(scrollToToday, 300); setTimeout(scrollToToday, 500);
    }
  ...
  const handleSave = async () => {
    ...
    if (isNaN(dureeNum) || dureeNum < 1) { alert('La durée doit être un nombre positif'); return; }

    const nouvelleDatatefin = calculateDateFinLogic(formData.datedebut, dureeNum);
    setIsSaving(true);

    try {
      await updateTache(selectedTask.id, {
        nom: selectedTask.nom, description: selectedTask.description,
        chantierid: selectedTask.chantierid, lotid: selectedTask.lotid,
        assigneid: selectedTask.assigneid, assignetype: selectedTask.assignetype,
        ...
        <div className="flex mt-10">
          <div className="flex-shrink-0 bg-slate-200" style={{ width: chantierColWidth }}>
            ...
            <div className="border-r border-slate-300">
              {items.map((chantier) => (
                <div key={`label-${chantier.id}`} className="flex items-center px-2 font-semibold text-slate-800 bg-slate-100 border-b border-slate-200/60" style={{ height: ROW_HEIGHT }}>
                  <Link to={`/chantiers/${chantier.id}`} className="truncate hover:underline text-xs w-full" title={chantier.name}>{chantier.name}</Link>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ minWidth: totalWidth }}>
              ...
              <div className="flex sticky top-7 bg-slate-100 z-20 border-b border-slate-300" style={{ gap: `${DAY_GAP}px` }}>
                {weekHeaders.map((week, idx) => (
                  <div key={`week-${idx}`} className="h-6 flex items-center justify-center border-r border-slate-200/80 bg-white" style={{ width: week.daysCount * (dayWidth + DAY_GAP) - DAY_GAP }}>
                    <span className="text-[10px] font-medium text-slate-700">{week.label}</span>
                  </div>
                ))}
              </div>
              <div className="flex sticky top-13 bg-slate-50 z-20 border-b border-slate-300" style={{ gap: `${DAY_GAP}px` }}>
                {allDays.map((day, idx) => {
                  ...
                  const isWE = isWeekend(day); const isFerie = isJourFerie(day);
                  return (
                    <div key={`day-header-${idx}`} className={`h-5 flex flex-col items-center justify-center text-[9px] font-medium ${isTargetDay ? 'bg-blue-500 text-white font-bold' : (isWE || isFerie) ? 'bg-gray-300 text-slate-600' : 'bg-white text-slate-600'}`}
                      style={{ width: dayWidth }} title={format(day, 'dd MMM yyyy', { locale: fr })}>
                      <span className="leading-none">{format(day, 'EEEEE', { locale: fr })}</span>
                      <span className="leading-none text-[8px] opacity-70">{format(day, 'd')}</span>
                    </div>
                  );
                })}
              </div>
                ...
                {allDays.map((day, i) => {
                  ...
                  return (
                    <React.Fragment key={`gridline-v-${i}`}>
                      {(isWE || isFerie) && <div className="absolute bg-gray-200" style={{ left: getDayPosition(i), top: 0, width: dayWidth, height: chartHeight - 100, zIndex: 1 }} />}
                      {isTargetDay && !isWE && <div className="absolute bg-blue-200 opacity-30" style={{ left: getDayPosition(i), top: 0, width: dayWidth, height: chartHeight - 100, zIndex: 2 }} />}
                      <div className="absolute border-l border-slate-200/40" style={{ left: getDayPosition(i), top: 0, bottom: 0, height: chartHeight - 100, zIndex: 3 }} />
                    </React.Fragment>
                  );
                })}
              </div>
                ...
                {items.map((chantier, chantierIndex) => {
                  ...
                  return (
                    <React.Fragment key={`chantier-row-${chantier.id}`}>
                      <div className="absolute left-0 right-0 border-b border-slate-200/60" style={{ top: rowTop + ROW_HEIGHT - 1, height: 1, zIndex: 0 }} />
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2 p-3 bg-slate-50 rounded-md border">
                  ...
                  {selectedTask.assignetype === 'soustraitant' && selectedTask.assigneid && (
                    <div><Label className="text-xs text-slate-500">Sous-traitant</Label><p className="font-medium text-orange-700">👷 {getArtisanNom(selectedTask.assigneid)}</p></div>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <Label htmlFor="datedebut">Date de début <span className="text-red-500">*</span></Label>
                    <Input id="datedebut" type="date" value={formData.datedebut} onChange={(e) => setFormData(prev => ({ ...prev, datedebut: e.target.value }))} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="duree">Durée (jours) <span className="text-red-500">*</span></Label>
                    <Input id="duree" type="number" min="1" value={formData.duree} onChange={(e) => setFormData(prev => ({ ...prev, duree: e.target.value }))} />
                  </div>
                </div>
                {formData.datedebut && formData.duree && (
                  <div className="text-sm text-slate-600 bg-blue-50 p-2 rounded">
                    📅 Date de fin calculée : <span className="font-semibold">{format(parseISO(calculateDateFinLogic(formData.datedebut, parseInt(formData.duree, 10))), 'dd/MM/yyyy', { locale: fr })}</span>
                  </div>
                )}
              </div>
              <DialogFooter>
